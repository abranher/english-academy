...
import { getEnrollmentOrders } from "@/services/network/enrollment-orders";
...
export function AllEnrollmentOrders({ studentId }: { studentId: string }) {
  const {
    isPending,
    data: enrollmentOrders,
    isError,
  } = useQuery<(EnrollmentOrder & { course: Course })[] | []>({
    queryKey: ["student_enrollment_orders"],
    queryFn: () => getEnrollmentOrders(studentId),
  });
